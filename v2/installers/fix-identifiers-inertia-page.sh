...
BACKUP="v2/backups/identifiers-inertia-fix/$STAMP"

CONTROLLER="app/Http/Controllers/V2/Admin/IdentifierController.php"
PAGE="resources/js/Pages/V2/Admin/Identifiers/Index.jsx"

mkdir -p \
    "$BACKUP" \
    "$(dirname "$PAGE")"

cp -a "$CONTROLLER" "$BACKUP/IdentifierController.php"

if [ -f "$PAGE" ]; then
    cp -a "$PAGE" "$BACKUP/Index.jsx"
fi

echo "=================================================="
echo "FIXING ISRC & UPC INERTIA MODULE"
echo "=================================================="

echo "[1/5] Fixing IdentifierController pending response..."
...
path = Path(
    "app/Http/Controllers/V2/Admin/IdentifierController.php"
)

text = path.read_text()

if "use Inertia\\Inertia;" not in text:
    text = text.replace(
...
replacement = """    public function pending(
        Request $request,
        PermissionService $permissions
    ): Response {
...
if count != 1:
    raise SystemExit(
        "IdentifierController pending() method का expected block नहीं मिला।"
    )

path.write_text(new_text)

print("Identifier pending route अब Inertia page return करेगी।")
PY

echo "[2/5] Creating Identifiers frontend page..."
...
echo "[3/5] Running PHP syntax check..."
...
echo ""
echo "===== IDENTIFIER ROUTE ====="

php artisan route:list |
grep "v2/admin/identifiers/pending"

echo ""
echo "=================================================="
echo "ISRC & UPC INERTIA MODULE FIXED"
echo "=================================================="

echo "Open:"
echo "https://admin.mixxtune.com/v2/admin/identifiers/pending"
