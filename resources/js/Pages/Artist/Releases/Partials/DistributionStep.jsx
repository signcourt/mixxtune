...
export default function DistributionStep({
    ...
    const defaultStoreIds = useMemo(
        () =>
            distributionStores
                .filter((store) => Boolean(store.default_selected))
                .map((store) => Number(store.id)),
        [distributionStores]
    );

    const existingStoreIds = Array.isArray(release?.stores)
        ? release.stores.map(Number)
        : [];

    const existingTerritories = Array.isArray(release?.territories)
        ? release.territories
        : [];
    ...
    const [releaseTimezone, setReleaseTimezone] = useState(
        release?.release_timezone || 'Asia/Kolkata'
    );
    ...
    const selectAllStores = () => {
        setSelectedStores(
            distributionStores.map((store) => Number(store.id))
        );
    };
    ...
    const saveDistribution = () => {
        if (!release?.id) {
            setMessage('Release must be saved before selecting stores.');
            return;
        }

        if (selectedStores.length === 0) {
            setMessage('Select at least one distribution store.');
            return;
        }

        if (!worldwide && territories.length === 0) {
            setMessage(
        ...
        router.patch(
            `/releases/${release.id}/distribution`,
            {
    ...
    return (
        <div className="space-y-7 p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                        Select the platforms and territories where this
                        release should be delivered.
                    </p>
                </div>
                ...
                <div className="mb-5 flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            ...
                            {distributionStores.length} stores selected
                        </p>
                    </div>
                ...
                {distributionStores.length === 0 ? (
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {distributionStores.map((store) => {
                            const selected = selectedStores.includes(
                                Number(store.id)
                            );
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <p className="mt-1 text-sm text-slate-500">
                    Choose worldwide delivery or selected countries.
                </p>
                ...
                <label className="mt-5 flex cursor-pointer items-center justify-between rounded-2xl border border-slate-200 p-4">
                    <div>
                        ...
                        <div className="mt-1 text-sm text-slate-500">
                            Deliver the release in every supported
                            territory.
                        </div>
                    </div>
                ...
                {!worldwide && (
                    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                        {territoryOptions.map((territory) => {
                            const selected =
                                territories.includes(territory.code);
                ...
                <div className="mt-5 grid gap-5 md:grid-cols-2">
                    ...
                    <label className="flex cursor-pointer items-center justify-between rounded-2xl border border-slate-200 p-4">
                        <div>
                            ...
                            <div className="mt-1 text-sm text-slate-500">
                                Allow pre-order where supported by the
                                selected stores.
                            </div>
                        </div>
            ...
            <div className="flex justify-end">
                <button
                    ...
                    disabled={processing}
                    className="rounded-xl bg-violet-600 px-6 py-3 text-sm font-semibold text-white shadow-sm hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
                    {processing
                        ? 'Saving Distribution...'
